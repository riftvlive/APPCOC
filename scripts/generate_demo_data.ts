...
// Complete Moroccan Poultry ERP Demo & Random Data Generator
export function generateComprehensiveData() {
  ...
  // Generate realistic daily logs
  const dailyLogs: any[] = [];

  // Helper to generate daily progression
  function generateCycleLogs(cycleId: string, startDateStr: string, daysCount: number, chickCount: number, breedTargetWeightKg: number) {
    ...
    for (let day = 1; day <= daysCount; day++) {
      ...
      // Realistic weight curve: day 1 is ~42g, day 40 is ~2350g
      const progressRatio = day / 42;
      const avgWeight = Math.round(42 + (breedTargetWeightKg * 1000 - 42) * Math.pow(progressRatio, 1.45));

      // Daily feed intake per bird in grams: day 1: ~15g, day 20: ~85g, day 35: ~165g
      ...
      // Mortality: brooding week 1 has slightly higher (10-25 birds), then normal low (3-12 birds)
      let mortality = 0;
      ...
      // Temperature: 33C at day 1 down to 23C at day 35
  ...
  // Cycle 1: 42 days (completed)
  generateCycleLogs('cycle-1', '2026-05-01', 42, 20000, 2.22);

  // Cycle 2: 39 days (ready for sale)
  generateCycleLogs('cycle-2', '2026-07-15', 39, 18000, 2.38);

  // Cycle 3: 24 days (active grower)
  generateCycleLogs('cycle-3', '2026-08-06', 24, 25000, 2.25);

  // Cycle 4: 11 days (active starter)
  generateCycleLogs('cycle-4', '2026-08-19', 11, 15000, 2.15);

  // Cycle 5: 2 days (new reception)
  generateCycleLogs('cycle-5', '2026-08-29', 2, 12000, 2.20);

  const chickPurchases = [
    {
      id: 'chk-1',
  ...
  const feedPurchases = [
    // Completed Cycle 1 Feeds (Total: 67,000 kg)
    {
      id: 'feed-1',
    ...
    // Active Cycle 2 Feeds (Total: 59,000 kg)
    {
      id: 'feed-4',
  ...
  const sales = [
    // Completed Cycle 1 Sales (Total sold: 19,240 birds, 42,328 kg, Net: 770,500 DH)
    {
      id: 'sale-1',
    ...
    // Fresh small batch sale from Cycle 2
    {
      id: 'sale-4',
...
if (process.argv[1] && process.argv[1].endsWith('generate_demo_data.ts')) {
  ...
  console.log(`Successfully seeded app_state.json with:`);
  console.log(`- ${data.farms.length} Farms`);
  console.log(`- ${data.cycles.length} Cycles`);
  console.log(`- ${data.dailyLogs.length} Daily Logs`);
  console.log(`- ${data.partners.length} Partners (Suppliers & Customers)`);
  console.log(`- ${data.accounts.length} Accounts`);
  console.log(`- ${data.workers.length} Workers`);
  console.log(`- ${data.chickPurchases.length} Chick Purchases`);
  console.log(`- ${data.feedPurchases.length} Feed Purchases`);
  console.log(`- ${data.medicationPurchases.length} Medication Purchases`);
  console.log(`- ${data.sales.length} Sales`);
  console.log(`- ${data.expenses.length} Expenses`);
  console.log(`- ${data.transactions.length} Financial Transactions`);
}
