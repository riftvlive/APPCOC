...
export const SalesView: React.FC<SalesViewProps> = ({ onOpenQuickAction }) => {
  ...
  const filteredSales = sales.filter(s => {
    // Top header filter if set
    if (selectedFarmId !== 'all') {
      if (s.farmId !== selectedFarmId) return false;
    }
    // Local source filter
    if (sourceFilter !== 'all') {
      if (sourceFilter === 'central') {
        if (s.farmId !== 'central') return false;
      } else if (s.farmId !== sourceFilter) {
        return false;
      }
    }
    // Payment filter
    if (paymentFilter === 'paid' && s.remainingAmount > 0) return false;
    if (paymentFilter === 'unpaid' && s.remainingAmount <= 0) return false;
    // Search query
    if (searchQuery.trim()) {
      ...
      if (!custName.includes(q) && !inv.includes(q) && !trk.includes(q)) return false;
    }
    return true;
  });
  ...
  return (
    <div className="print-invoice-host space-y-5 animate-fade-in pb-12">
      {/* Header */}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-stone-900 border border-stone-800 rounded-2xl p-3 sm:p-4 no-print">
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto flex-1">
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            ...
            <input
              ...
              placeholder="بحث برقم الفاتورة، اسم الزبون، لوحة الشاحنة..."
              className="w-full pl-3 pr-9 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs text-stone-100 placeholder-stone-500 focus:border-emerald-500 focus:outline-none"
            />
          </div>

          <div className="flex items-center gap-1.5 bg-stone-950 border border-stone-800 rounded-xl px-2.5 py-1.5">
            <Filter className="w-3.5 h-3.5 text-stone-400 shrink-0" />
            <span className="text-[11px] text-stone-400 font-bold ml-1">المصدر:</span>
            <select
              value={sourceFilter}
              onChange={e => setSourceFilter(e.target.value)}
              className="bg-transparent text-xs text-stone-200 font-semibold focus:outline-none cursor-pointer"
            >
              <option value="all" className="bg-stone-900">كل المصادر (المزارع والمخزن)</option>
              <option value="central" className="bg-stone-900 text-amber-300 font-bold">🏢 المخزن العام (المستودع المركزي)</option>
              {farms.map(f => (
                <option key={f.id} value={f.id} className="bg-stone-900">📍 {f.name}</option>
              ))}
            </select>
          </div>
          ...
          <select
            ...
            className="px-3 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs text-stone-200 focus:border-emerald-500 focus:outline-none"
          >
            <option value="all">كل حالات السداد</option>
            <option value="paid">مسددة بالكامل ✓</option>
            <option value="unpaid">عليها متبقي آجل</option>
          </select>
        </div>

        <div className="text-xs text-stone-400 font-bold shrink-0">
          عدد الفواتير: <span className="text-emerald-400 font-mono font-black">{filteredSales.length}</span>
        </div>
      </div>
          ...
          <table className="w-full text-right text-xs">
            <thead>
              <tr className="border-b border-stone-800 text-stone-400 text-[11px]">
                ...
                <th className="pb-3 font-bold">المصدر / المستودع</th>
                        ...
                        {s.farmId === 'central' ? (
                          <div className="flex flex-col items-start gap-0.5">
                            ...
                            <span className="text-[10px] text-stone-400 font-medium">
                              {s.cycleId === 'central_stock' ? 'مستودع التجميع والوزن' : (cycles.find(c => c.id === s.cycleId)?.cycleNumber || 'مبيعات مباشرة')}
                            </span>
                          </div>
                        ) : (
                          <div className="flex flex-col items-start gap-0.5">
                            <span className="text-stone-200 font-semibold">{farm?.name || 'المخزن العام'}</span>
                            {cycle && <span className="text-amber-400/90 font-bold text-[10px]">{cycle.cycleNumber}</span>}
                          </div>
                        )}
                      </td>
                      <td className="py-3">
      ...
      {selectedInvoice && (
        <div className="print-invoice fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
          ...
          <div className="relative w-full max-w-lg bg-white text-stone-900 border border-stone-300 rounded-2xl shadow-2xl overflow-hidden z-10 p-6 space-y-4 font-sans">
            <div className="flex items-start justify-between border-b pb-4">
              <div>
                <div className="text-[10px] font-bold text-amber-700 mb-1">مزارعنا لإدارة مزارع الدواجن</div>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              ...
              <div className="bg-stone-50 p-3 rounded-xl border">
                <span className="text-stone-500 block mb-1 font-bold">بيانات المصدر والشاحنة:</span>
                <span className="font-bold block text-stone-900">
                  {selectedInvoice.farmId === 'central' ? '🏢 المخزن العام (المستودع المركزي)' : (farms.find(f => f.id === selectedInvoice.farmId)?.name || 'المخزن العام')}
                </span>
                <span className="text-stone-600 block">
                  الدفعة / الدورة: {selectedInvoice.cycleId === 'central_stock' ? '📦 مخزون التجميع والتسويق بالمخزن العام' : (cycles.find(c => c.id === selectedInvoice.cycleId)?.cycleNumber || 'المخزن العام (بدون دورة)')}
                </span>
                <span className="text-stone-600 block">
                ...
                {selectedInvoice.driverName && (
                  <span className="text-stone-600 block">
                    السائق: {selectedInvoice.driverName}
                  </span>
                )}
              </div>
            </div>
